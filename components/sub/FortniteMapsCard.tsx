...
import type { MapStatus } from "@/constants/maps";
...
interface Props {
  ...
  notes?: string;
  /**
   * Status key rather than a display string, so styling stays keyed off a
   * stable value instead of substring-matching translated text.
   */
  status?: MapStatus;
  /** Already-formatted "August 2026" style label for the current locale. */
  updated?: string;
}

const STATUS_CLASS: Record<MapStatus, string> = {
  live: "border-emerald-400/50 bg-emerald-400/15 text-emerald-300",
  testing: "border-neon-magenta/50 bg-neon-magenta/15 text-neon-magenta",
  comingSoon: "border-amber-300/50 bg-amber-300/15 text-amber-200",
};
      ...
      <div className="clip-corner relative flex h-full flex-col bg-ink-800 transition-transform duration-300 group-hover:-translate-y-1">
        ...
        <div className="relative aspect-video w-full overflow-hidden bg-ink-700">
          ...
          <div className="absolute left-3 top-3 flex flex-wrap gap-2">
            ...
            {status && (
              <span
                className={`clip-corner-sm border px-2.5 py-1 text-[11px] font-black uppercase tracking-wider backdrop-blur-sm ${STATUS_CLASS[status]}`}
              >
                {ts(status)}
              </span>
            )}
